...
class LinearLayer_LoRA(nn.Module):
    ...
    def reset_parameters(self):
        ...
        #print('hack in dschat/utils/module/lora.py:64, to remove inconsistance to debug')
        #nn.init.zeros_(self.lora_right_weight)
        nn.init.zeros_(self.lora_left_weight)

    def fuse_lora_weight(self):
        #print('########## call fuse_lora_weight')
        if not self.fuse_lora:
            #print('########## call fuse_lora_weight: correct')
            self.weight.data += self.lora_scaling * torch.matmul(
                self.lora_left_weight.t(), self.lora_right_weight.t())
        else:
            #print('########## call fuse_lora_weight: incorrect')
            pass
        self.fuse_lora = True

    def forced_unfuse_lora_weight(self):
        # in resuming, forced to unfuse, because loading weights already unfused.
        if True:
            self.weight.data -= self.lora_scaling * torch.matmul(
                self.lora_left_weight.t(), self.lora_right_weight.t())
        else:
...
# unfuse the LoRA layer
# copied from convert_lora_to_linear_layer
# change module.fuse_lora_weight()->module.unfuse_lora_weight()
...
def forced_unfuse_lora_layer(model):
    replace_name = []
    for name, module in model.named_modules():
        if isinstance(module, LinearLayer_LoRA):
            replace_name.append(name)
    for name in replace_name:
        ...
        with deepspeed.zero.GatheredParameters(_z3_params_to_fetch([
                ...
                module.lora_right_weight
                                               ...
                                               modifier_rank=0,
                                               enabled=zero_stage_3):
            module.forced_unfuse_lora_weight()
    return model
