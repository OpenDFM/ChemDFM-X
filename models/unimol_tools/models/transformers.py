...
class TransformerEncoderWithPair(nn.Module):
    ...
    def forward(
        ...
        if input_padding_mask is not None:
            ...
            #token_mask = 1.0 - input_padding_mask.to(torch.float16)
            token_mask = 1.0 - input_padding_mask.type_as(x)
        else:
            #token_mask = torch.ones_like(x_norm, device=x_norm.device)
            #token_mask = torch.ones_like(x_norm, dtype=torch.float16, device=x_norm.device)
            token_mask = torch.ones_like(x_norm, dtype=x_norm.dtype, device=x_norm.device)
