...
class UniMolModel(BaseUnicoreModel):
    ...
    # NOTE make a procedure to load in main
    ...
    def forward(
        ...
        **kwargs
        ...
        graph_attn_bias = get_dist_features(src_distance.type_as(x), src_edge_type)
        (
            encoder_rep,
            _,
            _,
...
class GaussianLayer(nn.Module):
    ...
    def forward(self, x, edge_type):
        ...
        mean = self.means(torch.tensor([0], dtype=torch.long, device=x.device)).type_as(x).view(-1)
        std = self.stds(torch.tensor([0], dtype=torch.long, device=x.device)).type_as(x).view(-1).abs() + 1e-5
        return gaussian(x, mean, std).type_as(x)
