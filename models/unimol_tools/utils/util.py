...
def batch_collate_fn(samples):
    ...
    for k in samples[0].keys():
        if k == 'src_coord':
            ...
            #FIXME the padding_idx is set to 0 for hard coding, it should related to the tokenizer, but we just put the correct hard value here.
            #v = pad_2d([torch.tensor(s[k]).long() for s in samples], pad_idx=self.padding_idx)
            v = pad_2d([torch.tensor(s[k]).long() for s in samples], pad_idx=0.0)
        elif k == 'src_distance':
            ...
            v = pad_1d_tokens([torch.tensor(s[k]).long() for s in samples], pad_idx=0)
        batch[k] = v 
    try:
